...
1. **대상 결정**: `$ARGUMENTS`가 있으면 해당 경로(파일 또는 디렉터리)의 `*.java`.
   없으면 git 변경분에서 수집 — `git diff --name-only HEAD -- '*.java'`에
   `git status --porcelain`의 미추적 `*.java`를 더한다. 대상이 없으면 그렇게 알리고 끝낸다.
